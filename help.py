...
from aider_vox import __version__, utils
...
def get_package_files():
    for path in importlib_resources.files("aider_vox.website").iterdir():
        if path.is_file():
            yield path
        elif path.is_dir():
...
def fname_to_url(filepath):
    ...
    if filepath.startswith("website/_includes/"):
        ...
        docid = "https://aider_vox.chat/" + filepath
...
def get_index():
    ...
    dname = Path.home() / ".aider_vox" / "caches" / ("help." + __version__)
    ...
    if dname.exists():
        ...
        for fname in get_package_files():
            ...
            doc = Document(
                text=importlib_resources.files("aider_vox.website")
                .joinpath(fname)
