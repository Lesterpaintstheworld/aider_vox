...
from aider_vox import models, prompts, voice
from aider_vox.help import Help, install_help_extra
from aider_vox.llm import litellm
from aider_vox.scrape import Scraper, install_playwright
from aider_vox.utils import is_image_file
...
class Commands:
    ...
    def cmd_undo(self, args):
        "Undo the last git commit if it was done by aider_vox"
        if not self.coder.repo:
            self.io.tool_error("No git repository found.")
            return
        ...
        if last_commit_hash not in self.coder.aider_vox_commit_hashes:
            self.io.tool_error("The last commit was not made by aider_vox in this chat session.")
            self.io.tool_error(
    ...
    def basic_help(self):
        ...
        self.io.tool_output()
        self.io.tool_output("Use `/help <question>` to ask questions about how to use aider_vox.")

    def cmd_help(self, args):
        "Ask questions about aider_vox"
        ...
        user_msg += """
# Announcement lines from when this session of aider_vox was launched:
