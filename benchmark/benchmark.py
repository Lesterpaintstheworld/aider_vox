...
BENCHMARK_DNAME = Path(os.environ.get("aider_vox_BENCHMARK_DIR", "tmp.benchmarks"))
...
def main(
    ...
    replay: str = typer.Option(
        None,
        "--replay",
        help="Replay previous .aider_vox.chat.history.md responses from previous benchmark run",
    ),
    ...
    no_aider_vox: bool = typer.Option(False, "--no-aider_vox", help="Do not run aider_vox"),
    ...
    if "aider_vox_DOCKER" not in os.environ:
...
def show_diffs(dirnames):
    ...
    for testcase in testcases:
        ...
        for outcome, dirname in zip(all_outcomes, dirnames):
            print(outcome, f"{dirname}/{testcase}/.aider_vox.chat.history.md")
...
def load_results(dirname):
    dirname = Path(dirname)
    all_results = [json.loads(fname.read_text()) for fname in dirname.glob("*/.aider_vox.results.json")]
    return all_results
...
def summarize_results(dirname):
    ...
    command = f"aider_vox --model {a_model}"
    ...
    for hsh in commit_hashes:
        ...
        try:
            version = subprocess.check_output(
                ["git", "show", f"{hsh}:aider_vox/__init__.py"], universal_newlines=True
            )
...
def get_replayed_content(replay_dname, test_dname):
    ...
    replay_fname = replay_dname / test_name / ".aider_vox.chat.history.md"
    dump(replay_fname)
...
def run_test(original_dname, testdir, *args, **kwargs):
    try:
        ...
        results_fname = testdir / ".aider_vox.results.json"
...
def run_test_real(
    ...
    history_fname = testdir / ".aider_vox.chat.history.md"
