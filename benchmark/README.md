...
# aider_vox code editing benchmark harness

aider_vox uses a "code editing" benchmark to quantitatively measure how well it works
...
See [this writeup for a longer discussion about the benchmark and how to interpret the results](https://aider_vox.chat/docs/benchmarks.html).
...
There are 3 main tasks involved in benchmarking aider_vox:
...
```
# Clone the aider_vox repo
git clone git@github.com:paul-gauthier/aider_vox.git

# Create the scratch dir to hold benchmarking results inside the main aider_vox dir:
cd aider_vox
...
# Inside the container, install aider_vox as a development build.
...
- `--model` is the name of the model, same as you would pass directly to `aider_vox`.
- `--edit-format` is the name of the edit format, same as you would pass directly to `aider_vox`. When working with an experimental LLM, I recommend starting with `whole`
...
[aider_vox leaderboard data files](https://github.com/paul-gauthier/aider_vox/blob/main/aider_vox/website/_data/).
...
[aider_vox leaderboard data files](https://github.com/paul-gauthier/aider_vox/blob/main/aider_vox/website/_data/).
- These scripts are not intended for use by typical aider_vox end users.
