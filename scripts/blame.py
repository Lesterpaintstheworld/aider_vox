    ...
    for file in files:
        ...
        if file_counts:
            ...
            for author, count in file_counts.items():
                ...
                if "(aider_vox)" in author.lower():
                    aider_vox_total += count

    total_lines = sum(grand_total.values())
    aider_vox_percentage = (aider_vox_total / total_lines) * 100 if total_lines > 0 else 0

    end_date = get_tag_date(end_tag if end_tag else "HEAD")

    return all_file_counts, grand_total, total_lines, aider_vox_total, aider_vox_percentage, end_date
...
def get_commit_authors(commits):
    ...
    for commit in commits:
        ...
        if commit_message.lower().startswith("aider_vox:"):
            author += " (aider_vox)"
...
def process_all_tags_since(start_tag):
    ...
    for i in tqdm(range(len(tags) - 1), desc="Processing tags"):
        ...
        all_file_counts, grand_total, total_lines, aider_vox_total, aider_vox_percentage, end_date = blame(
            start_tag, end_tag
        )
        results.append(
            {
                ...
                "aider_vox_total": aider_vox_total,
                "aider_vox_percentage": round(aider_vox_percentage, 2),
            }
        )
    return results
...
def main():
    parser = argparse.ArgumentParser(description="Get aider_vox/non-aider_vox blame stats")
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Find all tags since the specified tag and print aider_vox percentage between each pair of"
            " successive tags"
        ),
    )
    parser.add_argument(
    ...
    if not args.all_since:
        print(f"- aider_vox wrote {round(aider_vox_percentage)}% of the code in this release.")
