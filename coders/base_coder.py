...
from aider_vox import __version__, models, prompts, urls, utils
from aider_vox.commands import Commands
from aider_vox.history import ChatSummary
from aider_vox.io import InputOutput
from aider_vox.linter import Linter
from aider_vox.llm import litellm
from aider_vox.mdstream import MarkdownStream
from aider_vox.repo import GitRepo
from aider_vox.repomap import RepoMap
from aider_vox.sendchat import retry_exceptions, send_completion
from aider_vox.utils import format_content, format_messages, is_image_file
...
class Coder:
    ...
    repo = None
    last_aider_vox_commit_hash = None
    aider_vox_edited_files = None
    ...
    def create(
        ...
        **kwargs,
        ...
        import aider_vox.coders as coders
        ...
        if from_coder:
            ...
            update = dict(
                ...
                aider_vox_commit_hashes=from_coder.aider_vox_commit_hashes,
    ...
    def get_announcements(self):
        lines = []
        lines.append(f"aider_vox v{__version__}")
        ...
        if self.repo:
            ...
            if num_files > 1000:
                lines.append(
                    "Warning: For large repos, consider using --subtree-only and .aider_voxignore"
                )
    ...
    def __init__(
        ...
        self.aider_vox_commit_hashes = set()
        ...
        if aider_vox_commit_hashes:
            self.aider_vox_commit_hashes = aider_vox_commit_hashes
        else:
            self.aider_vox_commit_hashes = set()
        ...
        for fname in fnames:
            ...
            if self.repo and self.repo.ignored_file(fname):
                self.io.tool_error(f"Skipping {fname} that matches aider_voxignore spec.")
                continue
    ...
    def send_message(self, inp):
        self.aider_vox_edited_files = None
        ...
        if edited:
            self.aider_vox_edited_files = edited
    ...
    def lint_edited(self, fnames):
        ...
        if self.repo and self.auto_commits and not self.dry_run:
            commit_res = self.repo.commit(
                fnames=fnames, context="The linter made edits to these files", aider_vox_edits=True
            )
            if commit_res:
    ...
    def auto_commit(self, edited):
        ...
        res = self.repo.commit(fnames=edited, context=context, aider_vox_edits=True)
        if res:
    ...
    def show_auto_commit_outcome(self, res):
        commit_hash, commit_message = res
        self.last_aider_vox_commit_hash = commit_hash
        self.aider_vox_commit_hashes.add(commit_hash)
        self.last_aider_vox_commit_message = commit_message
        if self.show_diffs:
    ...
    def show_undo_hint(self):
        ...
        if self.commit_before_message[-1] != self.repo.get_head():
            self.io.tool_output("You can use /undo to undo and discard each aider_vox commit.")
