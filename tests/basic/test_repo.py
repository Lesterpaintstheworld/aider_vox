...
from aider_vox.utils import GitTemporaryDirectory
...
class TestRepo(unittest.TestCase):
    ...
    @patch("aider_vox.repo.simple_send_with_retries")
    ...
    @patch("aider_vox.repo.GitRepo.get_commit_message")
    def test_commit_with_custom_committer_name(self, mock_send):
        ...
        with GitTemporaryDirectory():
            ...
            git_repo.commit(fnames=[str(fname)], aider_vox_edits=True)

            # check the committer name
            commit = raw_repo.head.commit
            self.assertEqual(commit.author.name, "Test User (aider_vox)")
            self.assertEqual(commit.committer.name, "Test User (aider_vox)")

            # commit a change without aider_vox_edits
            fname.write_text("new content again!")
            git_repo.commit(fnames=[str(fname)], aider_vox_edits=False)
    ...
    def test_get_tracked_files_with_aider_voxignore(self):
        with GitTemporaryDirectory():
            # new repo
            raw_repo = git.Repo()
            ...
            aider_voxignore = Path(".aider_voxignore")
            git_repo = GitRepo(InputOutput(), None, None, str(aider_voxignore))
            ...
            aider_voxignore.write_text("new.txt\n")
            time.sleep(2)
            ...
            #
            # aider_voxignore.write_text("new2.txt\n")
