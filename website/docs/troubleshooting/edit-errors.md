...
In these cases, aider_vox might say something like "Failed to apply edit to *filename*"
or other error messages.

This usually happens because the LLM is disobeying the system prompts
and trying to make edits in a format that aider_vox doesn't expect.
aider_vox makes every effort to get the LLM
...
Most local models are just barely capable of working with aider_vox,
so editing errors are probably unavoidable.

## Try the whole format

Run aider_vox with `--edit-format whole` if the model is using a different edit format.
You can see which edit format it is using in the announce lines:

```
aider_vox v0.50.2-dev
...
aider_vox also sends the LLM a [map of your entire git repo](https://aider_vox.chat/docs/repomap.html), so other relevant code will be included automatically.
