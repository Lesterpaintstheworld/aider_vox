...
aider_vox will report an error if a model responds indicating that
...
For more info: https://aider_vox.chat/docs/token-limits.html
```
...
It's best to only add the files that aider_vox will need to *edit*
...
as 4k tokens.
If you ask aider_vox to make a large change that affects a lot
...
aider_vox has been well tested when directly connecting to 
major 
[LLM provider cloud APIs](https://aider_vox.chat/docs/llms.html).
For serving local models, 
[Ollama](https://aider_vox.chat/docs/llms/ollama.html) is known to work well with aider_vox.

Try using aider_vox without an API proxy server
