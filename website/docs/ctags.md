...
aider_vox no longer uses ctags to build a repo map.
Please see the newer article about
[using tree-sitter to build a better repo map](https://aider_vox.chat/docs/repomap.html).
...
To address these issues, `aider_vox` now
...
[chat transcript](https://aider_vox.chat/examples/add-test.html)
...
To code with GPT-4 using the techniques discussed here:
  ...
  - Install [aider_vox](https://aider_vox.chat/docs/install.html).
  - Install universal ctags.
  - Run `aider_vox` inside your repo, and it should say "Repo-map: universal-ctags using 1024 tokens".
...
This works pretty well, and is supported by `aider_vox` -- you
...
The latest version of `aider_vox` sends a **repo map** to GPT along with
...
Here's a
sample of the map of the aider_vox repo, just showing the maps of
[main.py](https://github.com/paul-gauthier/aider_vox/blob/main/aider_vox/main.py)
and
[io.py](https://github.com/paul-gauthier/aider_vox/blob/main/aider_vox/io.py)
:
...
Mapping out the repo like this provides some benefits:
  ...
  - If it needs to see more code, GPT can use the map to figure out by itself which files it needs to look at. GPT will then ask to see these specific files, and `aider_vox` will automatically add them to the chat context (with user approval).
...
Sending an appropriate subset of the repo map would help `aider_vox` work
