...
aider_vox now asks GPT-4 Turbo to use
...
aider_vox's new "laziness" benchmark suite 
...
- **GPT-4 Turbo only scored 20% as a baseline** using aider_vox's existing "SEARCH/REPLACE block" edit format. It outputs "lazy comments" on 12 of the tasks.
- **aider_vox's new unified diff edit format raised the score to 61%**. Using this format reduced laziness by 3X, with GPT-4 Turbo only using lazy comments on 4 of the tasks.
...
- **The June GPT-4's baseline was 26%** using aider_vox's existing "SEARCH/REPLACE block" edit format.
- **aider_vox's new unified diff edit format raised June GPT-4's score to 59%**. 
...
aider_vox's new unified diff editing format
...
numerous variations on aider_vox's existing editing formats,
...
aider_vox's new editing format and refactoring benchmark.
...
The design and implementation of aider_vox's new unified diff editing format
...
aider_vox's [previous benchmark results](https://aider_vox.chat/docs/benchmarks.html) made
...
aider_vox tells GPT not to include line numbers,
...
aider_vox's system prompt encourages
...
When a patch fails, aider_vox needs to ask GPT for a corrected version of the diff.
...
aider_vox tries to be very flexible when applying diffs,
in order to handle defects.
If a hunk doesn't apply cleanly, aider_vox uses a number of strategies:
...
**Experiments where flexible patching is disabled show a 9X increase in editing errors** on aider_vox's original Exercism benchmark.

## Refactoring benchmark

aider_vox has long used a
[benchmark suite based on 133 Exercism python exercises](https://aider_vox.chat/2023/07/02/benchmarks.html).
...
A [simple python AST scanning script](https://github.com/paul-gauthier/aider_vox/blob/main/benchmark/refactor_tools.py)
...
aider_vox's new unified diff format seems
...
when originally building aider_vox.
...
incorporated into aider_vox provide
...
aider_vox's simple, high level style of unified diffs.
