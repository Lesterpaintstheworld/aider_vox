...
In most cases, you can safely ignore aider_vox's warning about unknown context
window size and model costs.

But, you can register context window limits and costs for models that aren't known
to aider_vox. Create a `.aider_vox.model.metadata.json` file in one of these locations:

- Your home directory.
- The root if your git repo.
- The current directory where you launch aider_vox.
...
in the `.aider_vox.model.metadata.json` file.
For example, use `deepseek/deepseek-chat`, not just `deepseek-chat`.

## Model settings

aider_vox has a number of settings that control how it works with
different models.
...
a model that aider_vox doesn't know about.

To do that,
create a `.aider_vox.model.settings.yml` file in one of these locations:
