...
parent: Usage
nav_order: 50
description: Control aider_vox with in-chat commands like /add, /model, etc.
---
# In-chat commands

aider_vox supports commands from within the chat, which all start with `/`.

<!--[[[cog
from aider_vox.commands import get_help_md
cog.out(get_help_md())
]]]-->
...
| **/help** | Ask questions about aider_vox |
...
| **/undo** | Undo the last git commit if it was done by aider_vox |
...
To use vi/vim keybindings, run aider_vox with the `--vim` switch.
